...
const Post = () => {
  ...
  const { profileImg, name, body, img, id, owner_id }: any = router.query;
  const { data, isLoading } = useQuery(['liqwqw', id], () => fetchLikes(id));

  React.useEffect(() => {
    const post = posts.find((post: any) => post.img === img);
    if (post) {
  ...
  }, [data?.likes, id, img, posts, user?.id]);
  const savePost = () => {
    const item = posts.find((post: any) => post.img === img);

    if (item) {
      removePosts(item);
      setPostColor('text-green-700');
    } else {
      setPosts({ profileImg, name, body, img, id });
      setPostColor('text-green-700');
    }
  };
  ...
  return (
    <Container>
      <div className="flex flex-col text-zinc-600 mb-20">
        ...
        />
        <p className="p-5 lg:text-lg">{body}</p>
